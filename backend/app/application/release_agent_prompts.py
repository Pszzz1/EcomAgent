    ...
    return [
        ModelMessage(
            ...
            content=(
                ...
                "revision_target 只用于修改动作：修改文案时为 copy，修改宣传图时为 image，其他动作为空。"
                ...
                "在宣传图阶段使用 revise 时，必须把 revision_target 设为 copy；无法判断用户是在改文案还是改图片时，使用 clarify，"
                "不要把含糊的图片反馈强行解释成文案修改。"
        ...
        ModelMessage(
            ...
            content=(
                ...
                + user_message.strip()
                + "\n\n返回 JSON："
                "{intent,summary,task_updates,remove_requirement_ids,new_requirements,reactivate_requirement_ids,confirmation_resolutions,answer,question,target_revision,revision_target}。"
